...
function App() {
  ...
  useEffect(() => {
    if (loadSetupState().setupComplete) return;
  ...
  }, [setupState, syncSettings, persistSetupState, persistSyncSettings, updateChecklistItem]);
  ...
  const handleSyncNow = useCallback(async (isAutomatic = false) => {
    if (syncRetryTimerRef.current !== null) {
      window.clearTimeout(syncRetryTimerRef.current);
    ...
    try {
      ...
      const mergedEvents = [...currentEvents, ...result.events];
  ...
  }, [persistSyncSettings, updateEvents, updateSessions, emitEvent]);
  ...
  }, [setupState.setupComplete, handleSyncNow]);
  ...
  }, [setupState.setupComplete, syncSettings.autoSyncEnabled, handleSyncNow]);
  ...
  function startSession(input: CreateSessionInput): WorkSession {
    ...
    const nextEvents = dedupeEvents([...events, event]);
  ...
  function updateCard(cardId: string, patch: WorkCardPatch) {
    const nextCards = updateWorkCard(cards, cardId, patch);
    saveCards(nextCards);
    setCards(nextCards);
  ...
  function finishSession(sessionId: string, endNote?: string) {
    const nextSessions = endSession(sessions, sessionId, endNote);
    saveSessions(nextSessions);
    setSessions(nextSessions);
    const endedSession = nextSessions.find((session) => session.id === sessionId);
  ...
  function handleUpdateSessionNotes(sessionId: string, notes: string) {
    const nextSessions = updateSessionNotes(sessions, sessionId, notes);
    saveSessions(nextSessions);
    setSessions(nextSessions);
  ...
  const handlePullRepo = useCallback(async (path: string) => {
